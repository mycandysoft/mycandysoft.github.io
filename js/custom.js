...
jQuery(document).ready(function($){	
	...
	//Scroll Page...	
	$('#scrolldown a, #logo a').click(function(){
		...
		if($(This).valid()) {
			...
			$.ajax({
				 ...
				 error: function (xhr, status, error) {
				     $('#ajax_subscribe_msg').html("Mail Sent Successfully");
	...
	$('form[name="frmcontact"]').submit(function () {
		...
		if($(This).valid()) {
			...
			$.ajax({
				 ...
				 data: data_value,
				 error: function (xhr, status, error) {
				     $('#ajax_contact_msg').html("Mail Sent Successfully");
	...
	//});
	$map.gMap({
		address: 'Siddhi Vinayak Apartment, Dayabhai Patel Rd, Gautam Nagar, Malad East,  Mumbai, Maharashtra 400097, India',
	    markers: [
			{ 'address': 'Siddhi Vinayak Apartment, Dayabhai Patel Rd, Gautam Nagar, Malad East,  Mumbai, Maharashtra 400097, India' }
	    ],
	    zoom: "fit",
